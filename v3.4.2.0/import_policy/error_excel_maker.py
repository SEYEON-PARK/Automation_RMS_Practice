            ...
            for col in sheet.iter_cols(count, sheet.max_column, 1, 1):  # 첫 번째 행만 검사
                ...
                if header and column_name in str(header):  # 헤더가 '공격명'을 포함하는지 확인
                    ...
                    for i in range(len(each_sheet[kind_of_attack][sheet_name][column_name]['index'])):
                        ...
                        # txt_file.write(f'공격 코드 : {sheet[f'{index_key[sheet_index]}{each_sheet[kind_of_attack][sheet_name][column_name]['index'][i]}']}, 에러 메시지 : {each_sheet[kind_of_attack][sheet_name][column_name]['error_message'][i]}\n')  # 각 줄 뒤에 줄바꿈 문자 추가      
                    break
